...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const { toggleSettingsPanel } = useReaderStore();
    ...
    <div className="h-full flex flex-col bg-stone-50 dark:bg-stone-900/95 border-r border-stone-200 dark:border-stone-800 w-72 shrink-0">
        ...
        <div className="space-y-2">
          ...
          {onBackToLibrary && (
            <button
              ...
              className="w-full flex items-center gap-3 p-3.5 rounded-xl bg-stone-100 hover:bg-stone-200 dark:bg-stone-800 dark:hover:bg-stone-700 transition-colors text-left border border-stone-200 dark:border-stone-700"
            >
              <Library className="w-5 h-5 text-stone-600 dark:text-stone-300 shrink-0" strokeWidth={1.75} />
              <div>
                <p className="font-medium text-sm text-stone-700 dark:text-stone-200">{t('library.open_library')}</p>
                <p className="text-xs text-stone-500 dark:text-stone-400">{t('library.title')}</p>
              </div>
            </button>
          )}
        </div>
      ...
      <div className="p-4 border-t border-stone-200 dark:border-stone-800">
        <button
          ...
          className="flex items-center gap-2 text-sm text-stone-600 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-100 transition-colors rounded-lg py-1.5 px-2 hover:bg-stone-100 dark:hover:bg-stone-800 w-full text-left"
        >
